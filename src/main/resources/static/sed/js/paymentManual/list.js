...
$(document).ready(function () {
    ...
    //카운터 클릭
    $("#allState, #completeState, #requestState, #rejectState, #releaseState, #cancelState").on("click", function () {
        $("#allState, #completeState, #requestState, #rejectState, #releaseState, #cancelState").each(function () {
...
function requestCancelPaymentAct() {
    requestAjax("/se/manualPayment/requestCancelPaymentAct", "post", {id: $("#targetID").val()}, function (res) {
        if (res.success) {
            alert(res.message);
        }
        location.reload();
...
//해지
function releaseManualPaymentAct() {
    requestAjax("/se/manualPayment/releaseManualPaymentAct", "post", {id: $("#targetID").val()}, function (res) {
        if (res.success) {
            alert(res.message);
        }
        location.reload();
...
//재신청
function manualRequestResendAct(targetID) {
    requestAjax("/se/manualPayment/manualRequestResendAct", "post", {id: targetID}, function (res) {
        alert(res.message);
        location.reload();
    });
}
...
function checkBlockPayForRequest (blockPay, requestID) {
    blockPay !== 'true' ? manualRequestPop(requestID) : blockPayPopup();
}
function checkBlockPayForResend (blockPay, targetID) {
    blockPay !== 'true' ? manualRequestResendAct(targetID) : blockPayPopup();
}
